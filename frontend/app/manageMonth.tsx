...
const manageMonth = () => {
    ...
    useEffect(()=>{
        const customerSearch = async()=>{
          const customerList1= await fetch(`http://10.0.2.2:4000/group//fetchCustomers/${groupID}`)
    ...
    const sendingPaymentData = async()=>{
        try{
          const sendData= await fetch(`http://10.0.2.2:4000/payment/addPayment`,{
            method: 'POST',
            headers: {
    ...
    useEffect(()=>{
        const fetchMonthData=async()=>{
            try{
                const data= await fetch(`http://10.0.2.2:4000/group/monthlyPayment/${monthID}`)
                console.log(data.ok);
  ...
  return (
    <ScrollView>
        <View>
            <View style={[{ opacity: paymentFlag ? 0.3 : 1 },{
    marginBottom:40,}]}>
            ...
            <View>
                <DataTable>
                    <DataTable.Header>
                                  ...
                                  <DataTable.Title><Text style={styles.tableTitle}>Sent To</Text></DataTable.Title>
                    ...
                    </DataTable.Header>
                    {
                        paymentSheet.map((payment)=>(
                            <DataTable.Row key={payment._id}>
                                ...
                                <DataTable.Cell><Text>Temp</Text></DataTable.Cell>
            ...
            {paymentFlag && (
                <View style={styles.paymentBlock}>
                    <View style={styles.close}>
                        <FontAwesome name="close" size={25} onPress={()=>{setPaymentFlag(!paymentFlag)}}/>
                    </View>
                    <View style={styles.inputContainer}>
                        <View style={styles.inputholder}>
                            ...
                            <Dropdown
                             ...
                             value={customerName}
                             onChange={item=>[setCustomerName(item.customerName),setCustomerId(item._id)]}
                        ...
                        <View style={styles.inputholder}>
                            ...
                            <Dropdown
                             ...
                             onChange={item=>[setPaidToCustomer(item.customerName),setPaidToCustomerId(item._id)]}
            ...
            <TouchableOpacity onPress={paymentFlag ? sendingPaymentData: ()=>{setPaymentFlag(!paymentFlag)}}>
                <View style={styles.button}>
                    <Text style={[styles.title,{padding:10}]}>{paymentFlag ? "Done": <><FontAwesome6 name ="plus" size="20"/>Payment</>}</Text>
                </View>
            </TouchableOpacity>
        </View>
    </ScrollView>
