...
const Group = ({ selectedGroupID = [], customerID = "" }) => {
      ...
      if (inputGroupName && startinputDate && endinputDate) {
        try {
          const addGroup = await fetch(
            `http://0.0.0.0:4000/group/createGroup`,
            {
              method: "POST",
              headers: {
  ...
  useEffect(() => {
    const fetchGroupData = async () => {
      try {
        const response = await fetch("http://0.0.0.0:4000/group/fetchGroups");
        if (response.ok) {
          const data = await response.json();
          let groupsData = data["data"];

          if (selectedGroupID.length > 0) {
            const selectedGroupIDs = selectedGroupID.map(
              (group) => group.groupID
            );

            const finalData = groupsData.filter((item) =>
              selectedGroupIDs.includes(item._id)
            );
            ...
            <View key={group._id} style={styles.cardHead}>
              <View key={group._id} style={styles.card}>
                ...
                <View style={styles.rightView}>
                  ...
                  <AntDesign
                    key={group._id}
                    name={groupDrop[group._id] ? "caretup" : "caretdown"}
                    size={24}
                    color="white"
                    onPress={() => handleDropDown(group._id)}
                  />
                </View>
              </View>
            ...
            <View style={styles.button}>
              <Text style={[styles.title, { padding: 10 }]}>
                ...
                {createGroup ? (
                  ...
                  <>
                    <FontAwesome6 name="plus" size="20" /> CreateGroup
                  </>
                )}
              </Text>
            </View>
